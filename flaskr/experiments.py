...
def _protein_expression_check(id, step_id, buffer_only=False): 
    ...
    {'name':"Check hour", 'id':"check_time", 'type':"datetime-local", 'hours_precision':True, 'next_entry_in':timedelta(days=7)}, 
...
def _scheduling(id, step_id, buffer_only=False, type=0): 
    ...
    if current_step.name == scheduling_steps[0]:
        ...
        if "Ad Libitum" in current_step.name:
            ...
            last_food_collected = interprete(Entries.query.filter(Entries.step_id==previous_step.id, Entries.name=='Food collected during experiment').order_by(desc(Entries.id)).first())
        ...
        args = {'mouse':mouse, 'page_name':page_name, 'forms':scheduling_initialization_forms, 'weight_target_message':weight_target_message, 'food_amount': daily_food, 'scoring':True, 'scheduling':"pre_weight", 'soft_weight_target':weight_target, 'hard_weight_target':extreme_weight, 'ad_libitum':ad_libitum_option, 'enddable':True}
        return display(args, reload_step=current_step, buffer=buffer_only)   
...
def _non_scheduling_experiment(id, step_id, buffer_only=False):
    ...
    experiment_forms = [{'name':"experiment_info", 'id':"experiment_info", 'type':"info", 'value':"Score mouse and precise experiment protocol, length or other usefull information"},
    {'name':"Score", 'id':"score", 'type':"int"}, {'name':"Scoring hour", 'id':"score_time", 'type':"datetime-local", 'hours_precision':True, 'next_entry_in':timedelta(days=7)}]
